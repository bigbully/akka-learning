...
/**
  ...
  */
class LightArrayRevolverScheduler(threadFactory: ThreadFactory, ticksPerWheel:Int, tickDuration:FiniteDuration, ShutdownTimeout:FiniteDuration)
  extends Scheduler with Closeable {

  import org.akka.learning.util.Helpers._

  val WheelSize = ticksPerWheel
      .requiring(ticks ⇒ (ticks & (ticks - 1)) == 0, "ticks-per-wheel must be a power of 2")
  val TickDuration =
    tickDuration
      .requiring(_ >= 10.millis || !isWindows, "minimum supported akka.scheduler.tick-duration on Windows is 10ms")
  ...
  protected def waitNanos(nanos: Long): Unit = {
    ...
    val sleepMs = if (isWindows) (nanos + 4999999) / 10000000 * 10 else (nanos + 999999) / 1000000
  ...
  @volatile private var timerThread: Thread = threadFactory.newThread(new Runnable {
    ...
    override final def run =
      try nextTick()
        ...
        case t: Throwable ⇒
          println("exception on LARS’ timer thread:" + t.getMessage)
          stopped.get match {
            case null ⇒
              val thread = threadFactory.newThread(this)
              println("starting new LARS thread")
              try thread.start()
              catch {
                case e: Throwable ⇒
                  println("LARS cannot start new thread, ship’s going down!" + e.getMessage)
...
object LightArrayRevolverScheduler {
  ...
  /**
    * INTERNAL API
    */
  class TaskHolder(@volatile var task: Runnable, var ticks: Int, executionContext: ExecutionContext)
    extends TimerTask {
